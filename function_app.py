...
def structuring_transition(prompt: str) -> str:
    system_prompt = '''
        あなたは業務システムの画面関連情報を解析し、構造化されたMarkdownドキュメントを作成する専門家です。

        【タスク】
        提供されたExcelシートから画面一覧または画面遷移情報を抽出し、読みやすく整理してください。
        データの内容に応じて適切に判断し、構造化してください。
...
def create_integration_test_spec(prompt: str) -> str:
    system_prompt = '''
        ...
        【スコープ】
        構造化詳細設計書に記載されている画面のみを対象とします。
        - 手順：詳細設計書から対象画面を特定 → 画面遷移図から該当画面間の遷移のみ抽出
        - 例：設計書に「ログイン画面」「商品検索画面」のみある場合、遷移図に「管理画面」があっても使用しない

        【タスク】
        画面間をまたぐ業務シナリオを抽出し、具体的なテストデータを含むテストケースに展開してください。

        【テストデータ】
        - 画面の全入力項目に対して具体的な値を記述（項目を省略しない）
        - 例：ユーザーID="admin"、日付="2024-01-15"、金額="10000"
        - 設計書の制約に合わせた値を生成（例：桁数10桁なら"1234567890"）

        【出力形式】
        Markdown表（10列固定）：
        テストNo | 画面1 | 機能/操作/状況1 | 画面2 | 機能/操作/状況2 | 画面3 | 機能/操作/状況3 | 画面4 | 機能/操作/状況4 | 確認内容

        - 画面遷移が少ない場合は該当列を空欄
        - 4画面超える場合は複数テストケースに分割
        - 表形式のみ出力（説明文不要）

        【記述ルール】
        - 操作内容：画面の全入力項目に対して具体的な値を記述し、「ボタン名」を明記
        - 例：「営業所=xx、得意先コード=xx、見積日=xx、現場名=xx、請負内容=xxを入力し、「登録」ボタンをクリック」
        - 確認内容：「～であること」「～されること」で統一
    '''
    return call_llm(system_prompt, prompt)
